...
GstPadProbeReturn __mmstreamrecorder_audio_dataprobe_audio_mute(GstPad *pad, GstPadProbeInfo *info, gpointer u_data)
{
	...
	if (hstreamrecorder->astream_cb && buffer && mapinfo.data && mapinfo.size > 0) {
		MMAudioStreamDataType stream;

		/*_mmstreamrec_dbg_log("Call video steramCb, data[%p], Width[%d],Height[%d], Format[%d]",
								gst_buffer_n_memory(buffer), width, height, format);*/
		...
		if (hstreamrecorder->astream_cb)
			hstreamrecorder->astream_cb(&stream, hstreamrecorder->astream_cb_param);
	}
