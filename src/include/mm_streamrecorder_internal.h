...
} _MMStreamRecorderCommand;

/**
 * MMstreamrecorder command information
 */
typedef struct {
	MMHandleType handle;											/**< streamrecorder handle */
	_MMstreamrecorderCommandType type;									/**< Type of command */
} __MMstreamrecorderCmdInfo;
...
typedef struct {
	...
	gboolean video_disable;				/**< whether video is disabled or not when record */
...
typedef struct mmf_streamrecorder {
	...
	mm_streamrecorder_video_stream_callback vstream_cb;			/**< Video stream callback */
	void *vstream_cb_param;									/**< Video stream callback parameter */
	mm_streamrecorder_audio_stream_callback astream_cb;			/**< Audio stream callback */
	void *astream_cb_param;									/**< Audio stream callback parameter */
	mm_streamrecorder_video_capture_callback vcapture_cb;		/**< Video capture callback */
	...
	_MMStreamRecorderCommand cmd;								/**< information for command loop */
	...
	int use_zero_copy_format;								/**< Whether use zero copy format for input */
	int shutter_sound_policy;								/**< shutter sound policy */

	mm_streamrecorder_ini_t ini;
	//  _MMstreamrecorderEnumConvert enum_conv[ENUM_CONVERT_NUM];                   /**< enum converting list that is modified by ini info */
