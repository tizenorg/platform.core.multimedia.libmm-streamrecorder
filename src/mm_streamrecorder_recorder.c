...
int _mmstreamrecorder_create_recorder_pipeline(MMHandleType handle)
{
	...
	/* get audio disable */
	mm_streamrecorder_get_attributes(handle, NULL, MMSTR_AUDIO_DISABLE, &audio_disable, NULL);
	sc->audio_disable = audio_disable;

	_mmstreamrec_dbg_log("AUDIO DISABLE : %d", sc->audio_disable);
	...
	if (sc->audio_disable == FALSE) {
		...
		MMSTREAMRECORDER_ADD_BUFFER_PROBE(srcpad, _MMSTREAMRECORDER_HANDLER_VIDEOREC, __mmstreamrecorder_audio_dataprobe_audio_mute, hstreamrecorder);
...
int _mmstreamrecorder_create_encodesink_bin(MMHandleType handle, MMStreamRecorderEncodebinProfile profile)
{
	...
	if (hstreamrecorder->ini.encsink_bin_profile != MM_STREAMRECORDER_ENCBIN_PROFILE_AUDIO) {
		...
		if (rec_mode == MM_STREAMRECORDER_MODE_MEDIABUFFER) {
			...
			MMSTREAMRECORDER_SIGNAL_CONNECT(sc->encode_element[_MMSTREAMRECORDER_ENCSINK_SRC].gst, _MMSTREAMRECORDER_HANDLER_VIDEOREC, "need-data", _mmstreamrecorder_ready_to_encode_callback, hstreamrecorder);
		}
	...
	} else if (hstreamrecorder->ini.encsink_bin_profile == MM_STREAMRECORDER_ENCBIN_PROFILE_IMAGE) {
	...
	if (sc->audio_disable == FALSE && hstreamrecorder->ini.encsink_bin_profile != MM_STREAMRECORDER_ENCBIN_PROFILE_IMAGE) {
	...
	/* Mux */
	if (hstreamrecorder->ini.encsink_bin_profile != MM_STREAMRECORDER_ENCBIN_PROFILE_IMAGE) {
		switch (file_format) {
	...
	if (sc->audio_disable == FALSE && profile != MM_STREAMRECORDER_ENCBIN_PROFILE_IMAGE) {
	...
	case _MM_STREAMRECORDER_CMD_COMMIT:
		/* video recording command */
		{
			...
			if (hstreamrecorder->quick_device_close) {
				_mmstreamrec_dbg_warn("quick_device_close");
...
int _mmstreamrecorder_video_handle_eos(MMHandleType handle)
{
	...
	if (!(sc->ferror_send)) {
		ret = _mmstreamrecorder_add_locationinfo((MMHandleType) hstreamrecorder, finfo->fileformat);
		if (ret) {
			_mmstreamrec_dbg_log("Writing location information SUCCEEDED !!");
		} else {
			_mmstreamrec_dbg_err("Writing location information FAILED !!");
		}
	}
...
int _mmstreamrecorder_create_audiop_with_encodebin(MMHandleType handle)
{
	...
	/* set data probe function */
	srcpad = gst_element_get_static_pad(sc->encode_element[_MMSTREAMRECORDER_AUDIOSRC_SRC].gst, "src");
	MMSTREAMRECORDER_ADD_BUFFER_PROBE(srcpad, _MMSTREAMRECORDER_HANDLER_AUDIOREC, __mmstreamrecorder_audio_dataprobe_voicerecorder, hstreamrecorder);
	gst_object_unref(srcpad);
	srcpad = NULL;
...
int _mmstreamrecorder_push_videostream_buffer(MMHandleType handle, unsigned long timestamp, void *buffer, int size)
{
	...
	if (sc->encode_element[_MMSTREAMRECORDER_ENCSINK_SRC].gst) {
		...
		ret = gst_app_src_push_buffer((GstAppSrc *) sc->encode_element[_MMSTREAMRECORDER_ENCSINK_SRC].gst, (GstBuffer *)buffer);
		if (ret) {
...
int _mmstreamrecorder_push_audiostream_buffer(MMHandleType handle, unsigned long timestamp, void *buffer, int size)
{
	...
	if (sc->encode_element[_MMSTREAMRECORDER_AUDIOSRC_SRC].gst) {
		...
		newbuf = _mmstreamrecorder_buffer_convert_audio(handle, buffer, timestamp, size);
		if (newbuf == NULL) {
			_mmstreamrec_dbg_err("Audio gst_app_src_push_buffer %d", err);
			return MM_ERROR_STREAMRECORDER_BUFFER_CONVERT;
		}
		srcpad = gst_element_get_static_pad(sc->encode_element[_MMSTREAMRECORDER_AUDIOSRC_SRC].gst, "src");
		...
		err = gst_app_src_push_buffer((GstAppSrc *) sc->encode_element[_MMSTREAMRECORDER_AUDIOSRC_SRC].gst, newbuf);
