...
gboolean _mmstreamrecorder_gstreamer_init()
{
	...
	*argc = 1;
	...
	/* add gst_param */
	/* _mmstreamrecorder_conf_get_value_string_array(conf,
...
void _mmstreamrecorder_ready_to_encode_callback(GstElement *element, guint size, gpointer handle)
{
	mmf_streamrecorder_t *hstreamrecorder = MMF_STREAMRECORDER(handle);
	_MMStreamRecorderSubContext *sc = NULL;

	/* _mmstreamrec_dbg_err("need data"); */
